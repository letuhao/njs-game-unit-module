...
export abstract class SizeCalculationTemplate implements IUnitCalculationTemplate {
  ...
  constructor(context: UnitContext) {
    ...
    try {
      this.strategy = container.resolve(TOKENS.SIZE_UNIT_STRATEGY);
    } catch (error) {
      this.strategy = new SizeUnitStrategy(); // Fallback to direct instantiation
    }
  ...
  protected createValidators(): Array<RangeValidator | TypeValidator> {
    try {
      ...
      return [
        rangeValidator || new RangeValidator('SizeRangeValidator', 0, 10000, true),
  ...
  public resetStats(): void {
    ...
    this.logger.debug('SizeCalculationTemplate', 'resetStats', 'Statistics reset');
  }
