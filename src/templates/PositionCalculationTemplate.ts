...
export abstract class PositionCalculationTemplate implements IUnitCalculationTemplate {
  ...
  constructor(context: UnitContext) {
    ...
    try {
      this.strategy = container.resolve(TOKENS.POSITION_UNIT_STRATEGY);
    } catch (error) {
      this.strategy = new PositionUnitStrategy(); // Fallback to direct instantiation
    }
  ...
  protected createValidators(): Array<RangeValidator | TypeValidator> {
    try {
      ...
      return [
        rangeValidator || new RangeValidator('PositionRangeValidator', -10000, 10000, true),
