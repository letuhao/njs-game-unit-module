 ...
 * Creates appropriate adapters for legacy units using DI
 */
 ...
 * Manages all available unit adapters using DI
 */
...
/**
 * Unit Adapter Factory Implementation with DI
 */
export class UnitAdapterFactory implements IUnitAdapterFactory {
  private adapterTypes = new Map<string, new (...args: unknown[]) => IUnitAdapter>();
  ...
  createAdapter(legacyUnit: ILegacyUnit): IUnitAdapter | undefined {
    ...
    try {
      ...
      const positionAdapter = new positionAdapterClass(
        'legacy-position',
        'Legacy Position Adapter',
        legacyUnit
      );
      if (positionAdapter.canAdapt(legacyUnit)) {
        return positionAdapter;
      }

      // Try size adapter
      const sizeAdapter = new sizeAdapterClass(
        'legacy-size',
        'Legacy Size Adapter',
        legacyUnit
      );
      if (sizeAdapter.canAdapt(legacyUnit)) {
        return sizeAdapter;
      }
    } catch (error) {
      // Fallback to registered adapter types
      for (const [typeName, AdapterClass] of this.adapterTypes) {
        const adapter = new AdapterClass('legacy', typeName, legacyUnit);
  ...
  registerAdapter(
    ...
    this.adapterTypes.set(adapterType, adapterClass);
  }
  ...
  hasAdapter(legacyUnit: ILegacyUnit): boolean {
    return this.createAdapter(legacyUnit) !== undefined;
  }

  getBestAdapter(legacyUnit: ILegacyUnit): IUnitAdapter | undefined {
    return this.createAdapter(legacyUnit);
  }

  private initializeDefaultAdapters(): void {
    // Default adapters will be registered via DI container
