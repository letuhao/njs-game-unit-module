...
import { PositionValueStrategyRegistry } from './registry/PositionValueStrategyRegistry';
...
export class PositionUnitStrategy implements IUnitStrategy {
  ...
  private strategyRegistry: PositionValueStrategyRegistry;

  constructor() {
    this.strategyRegistry = PositionValueStrategyRegistry.getInstance();
  }
  ...
  private mapStringToPositionValue(input: string): PositionValue | null {
    const stringToPositionValueMap: Record<string, PositionValue> = {
    ...
    return stringToPositionValueMap[input.toLowerCase()] || null;
  }

  /**
   * Calculate position from PositionValue enum using registry
   */
  private calculatePositionValue(input: PositionValue, context: UnitContext): number {
    const positionValueStrategy = this.strategyRegistry.getPositionValueStrategy(input);
    return positionValueStrategy(context);
  }
