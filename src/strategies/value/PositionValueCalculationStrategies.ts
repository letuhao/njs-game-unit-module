...
import { AxisUnitStrategyRegistry } from '../registry/AxisUnitStrategyRegistry';
...
export class CenterPositionValueCalculationStrategy implements IPositionValueCalculationStrategy {
  ...
  readonly axisUnit = AxisUnit.X;
  private strategyRegistry: AxisUnitStrategyRegistry;

  constructor() {
    this.strategyRegistry = AxisUnitStrategyRegistry.getInstance();
  }
  ...
  calculate(
    ...
    // Use registry to get the appropriate strategy for the axis unit
    const axisStrategy = this.strategyRegistry.getAxisUnitStrategy(axisUnit);
    return axisStrategy(context);
  }
