# CalculateSizeCommand.ts - Refactoring Plan

## File: `src/commands/CalculateSizeCommand.ts`
...
```typescript
// BEFORE
export class CalculateSizeCommand implements IUnitCommand {
  constructor() {
    this.sizeCalculator = new SizeUnitCalculator();
  }
  
  public execute(input: ISizeStrategyInput): number {
    return this.sizeCalculator.calculate(input);
  }
}
...
container.bind(TOKENS.CALCULATE_SIZE_COMMAND)
  .to(CalculateSizeCommand)
  .inSingletonScope();
```
...
- **Requires**: SizeStrategyInput interface
