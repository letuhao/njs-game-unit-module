# RefactoredSizeUnitCalculator.ts - Refactoring Plan

## File: `src/classes/RefactoredSizeUnitCalculator.ts`

## Issues Identified

### 1. **SRP:CrossCuttingInsideCore** (Line 9)
- **WHY**: Logger import violates Single Responsibility Principle
- **WHAT**: Move logging to decorator or orchestration layer
- **WHERE**: Line 9 - `import { logger } from '../core/Logger';`
...
export class RefactoredSizeUnitCalculator {
  public calculate(input: ISizeStrategyInput): number {
    logger.debug('RefactoredSizeUnitCalculator', 'calculate', 'Starting calculation');
    // ... calculation logic
    logger.info('RefactoredSizeUnitCalculator', 'calculate', 'Calculation completed');
  }
}
...
container.bind(TOKENS.REFACTORED_SIZE_CALCULATOR)
  .to(RefactoredSizeUnitCalculator)
  .inSingletonScope();
```
