# RefactoredPositionUnitCalculator.ts - Refactoring Plan

## File: `src/classes/RefactoredPositionUnitCalculator.ts`

## Issues Identified

### 1. **SRP:CrossCuttingInsideCore** (Line 10)
- **WHY**: Logger import violates Single Responsibility Principle
- **WHAT**: Move logging to decorator or orchestration layer
- **WHERE**: Line 10 - `import { logger } from '../core/Logger';`
...
export class RefactoredPositionUnitCalculator {
  public calculate(input: IPositionStrategyInput): number {
    logger.debug('RefactoredPositionUnitCalculator', 'calculate', 'Starting calculation');
    // ... calculation logic
    logger.info('RefactoredPositionUnitCalculator', 'calculate', 'Calculation completed');
  }
}
...
container.bind(TOKENS.REFACTORED_POSITION_CALCULATOR)
  .to(RefactoredPositionUnitCalculator)
  .inSingletonScope();
```
